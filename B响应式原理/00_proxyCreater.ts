...
// Proxy 主要用于创建对象的代理，拦截对象的各种操作，确保对象具备响应性。它可以用于对象（包括数组、函数、类实例等），但不能用于基本数据类型（如字符串、数字、布尔值等），因为基本数据类型是不可变的，没有属性供拦截。
...
// Proxy 适用于复杂结构
// 由于 Proxy 主要用于对象的代理，所以它适用于复杂的数据结构，例如：
